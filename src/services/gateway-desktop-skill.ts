...
export function renderGatewayDesktopSkill(): string {
  return [
    ...
    '4. If precise positioning matters, move the mouse first and capture a cursor-visible screenshot before clicking.',
    ...
    '- `node ./.codex/skills/gateway-desktop/scripts/gateway-desktop.mjs screenshot --filename desktop-step.png --show-cursor true`',
...
function renderGatewayDesktopScript(): string {
  return [
    ...
    "    '  screenshot [--filename desktop-step.png] [--show-cursor true]',",
    "  ].join('\\n'));",
    '}',
    '',
    ...
    "    case 'screenshot':",
    '      return { filename: optionalString(parsed.filename), showCursor: booleanValue(parsed["show-cursor"] ?? parsed.showCursor) };',
    '    default:',
