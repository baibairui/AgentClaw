...
function renderFeishuOpsPlaybook(): string {
  return [
    ...
    '- 当前用户自己的日历 / 待办仍然走同一个 skill，只是改用个人命令：`calendar create-personal-event` / `task create-personal-task`。',
    '',
    '## Standard Workflow',
    '1. 只处理 DocX / Wiki 这类应用凭据可直接完成的飞书官方操作。',
    '2. DocX / Wiki：先做只读探测（优先 `wiki list-spaces`），确认鉴权和权限状态。',
    '3. DocX / Wiki：再执行写操作（如 `docx create` / `wiki create-node`)。',
    '4. DocX / Wiki：成功标准是真实返回 document_id / document_url / node token；只回 markdown 文本不算完成。',
    '5. DocX / Wiki：不要把用户引到任何个人授权或登录页面。',
    '6. 输出真实返回（document_id/node token/错误码），不要编造成果。',
    '',
    '## Error Classification',
    '- 缺少凭据：`FEISHU_APP_ID` / `FEISHU_APP_SECRET` 未配置。',
    '- `99991672`：应用缺少对应 OpenAPI scope，需要管理员先开通权限。',
    '- `99991679`：用户授权未覆盖所需 scope；优先运行 `auth diagnose-permission`，区分“应用没开权限”与“用户需要按新 scope 重授权”。',
    '- 接口可用：只读探测成功，可继续写操作。',
    '- 个人日历 / 个人任务缺少授权：走带 `required-scopes-json` 的 `auth start-device-auth` -> `auth poll-device-auth` -> 重试 `calendar create-personal-event` 或 `task create-personal-task`。',
    '',
    '## Guardrails',
    '- 未实际调用 API 前，不得说“已创建”。',
