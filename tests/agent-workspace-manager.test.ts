...
describe('AgentWorkspaceManager', () => {
  it('creates workspace scaffold and global memory files', () => {
    ...
    expect(fs.existsSync(path.join(result.workspaceDir, 'skills', 'reminder-skill', 'SKILL.md'))).toBe(true);
    expect(fs.existsSync(path.join(result.workspaceDir, 'skills', 'reminder-skill', 'scripts', 'build-reminder-action.js'))).toBe(true);
  ...
  it('creates scaffold for skill onboarding template', () => {
    ...
    const reminderSkill = fs.readFileSync(path.join(result.workspaceDir, 'skills', 'reminder-skill', 'SKILL.md'), 'utf8');
    ...
    expect(reminderSkill).toContain('reminder-action');
  });
