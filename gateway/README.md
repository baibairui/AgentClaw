...
- 运行目录：`/opt/gateway`
- Codex 工作目录：`/opt/gateway/workspace/wecom-codex-gateway`

也就是让 Codex 只改 workspace，不直接改线上运行目录。等 workspace 改好后，再在服务器上执行发布：
...
发布脚本位于 `bin/publish-workspace.sh`，默认会执行这些步骤：

1. 备份当前 `/opt/gateway` 到 `/opt/deploy-backups/gateway-时间戳.tgz`
2. 将 `/opt/gateway/workspace/wecom-codex-gateway` 同步到 `/opt/gateway`
3. 保留 `/opt/gateway/.env`、`/opt/gateway/.data/`、`/opt/gateway/workspace/`
4. 执行 `npm ci`
5. 执行 `npm test`
6. 执行 `npm run build`
7. 执行 `pm2 restart wecom-codex --update-env`
8. 检查 `http://127.0.0.1:3000/healthz`

脚本失败时会立即停止，不自动回滚；备份会保留在 `/opt/deploy-backups`。
